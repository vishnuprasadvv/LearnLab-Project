...
import { startLoading, authSuccess, setError, endLoading } from '../features/authSlice';
...
const Signup = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit : async(values) => {
      ...
      try {
        ...
        const result =  dispatch(register({  firstName: values.firstName, lastName:values.lastName, email: values.email, password: values.password , phone: values.phone })).unwrap();
  ...
  return (
    <form onSubmit={formik.handleSubmit} className='md:w-1/3 lg:w-1/4 sm:w-1/2  items-center mx-auto border rounded-md p-6 mt-10'>
      <h1 className='text-2xl font-bold text-blue-600 text-center p-4 '>Sign Up</h1>
      <Toaster/>
      ...
      <div className='flex justify-center pt-5'>
        <Button className='bg-blue-600 rounded-full w-full hover:bg-blue-700' type="submit" disabled={loading}>Signup</Button>
      </div>
      <div className='flex justify-center pt-5'>
      ...
      <div className='flex justify-center pt-5'>
        ...
        <Link to={'/login'} className='pl-2 font-bold text-blue-600'><span >Login</span></Link>
      </div>
