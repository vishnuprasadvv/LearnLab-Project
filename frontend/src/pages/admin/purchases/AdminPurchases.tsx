            ...
            <tbody>
             ...
             even:dark:bg-gray-800 border-b dark:border-gray-700"
                >
                  ...
                  <td className="lg:px-6 px-1 py-4"> {`${order.userId.firstName} ${order.userId.lastName}`}</td>
                  <td className="lg:px-6 px-1 py-4"> {order.courses.flatMap((course) => course.courseInstructor)}</td>
                  <td className="lg:px-6 px-1 py-4">
                   {order.paymentDate && new Date(order.paymentDate).toDateString() || 'NA'}
                  </td>
                  <td className="lg:px-6 px-1 py-4">
                    <div>₹ {order.totalAmount}</div>
                  </td>
                  <td className="lg:px-6 px-1 py-4">
                    <div className={`uppercase text-xs text-white text-center rounded-full px-1 py-1 ${order.paymentStatus == 'completed' ? 'bg-green-500' : order.paymentStatus =='pending' ? 'bg-yellow-400' : 'bg-red-500'}`}>{order.paymentStatus}</div>
                  </td>
