...
const ratingSchema = z.object({
    rating: z.number().min(0.5, 'Rating must be atleast 0.5')
    .max(5, 'Rating cannot exceed 5'),
    review: z.string().min(1, 'Add your feedback.').max(500, 'Max 500 characters')
})

const CourseRatingComponent:React.FC<{courseId: string}> = ({courseId}) => {
        ...
        <div className="flex flex-col gap-2">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <Rating size="small" defaultValue={0} value={rating} precision={0.5} onChange={(e,newValue) => setRating(newValue)}/>
            </div>
          </div>
          <Textarea className="ml-10 w-auto" cols={50} 
          value={review} 
          onChange={(e) => setReview(e.target.value)}
          placeholder="Write your review here..."
          />
          <Button className="w-max place-self-end rounded-full bg-blue-600"
