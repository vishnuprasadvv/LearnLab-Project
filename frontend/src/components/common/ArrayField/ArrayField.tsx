...
const ArrayField: React.FC<ArrayFiledInterface> = ({ name, label, values, onChange }) => {
  ...
  return (
    <div>
      ...
      <div className='flex'>
        <Input type='text' value={inputValue} id={name} onChange={(e) => setInputValue(e.target.value)} placeholder={label} />
        <Button size={'icon'} variant={'outline'} className='ml-1'  type='button' onClick={handleAdd}><IoAdd/></Button>
      </div>
      <div className=' max-w-full overflow-y-auto mt-1' >
        <ul className='flex flex-wrap max-w-full overflow-x-auto'>
          {values.map((value, index) => (
            <li key={index}>
              <div className='bg-slate-100 p-1 rounded-md m-1 flex gap-2 text-xs'>
                <span>{value}</span>
