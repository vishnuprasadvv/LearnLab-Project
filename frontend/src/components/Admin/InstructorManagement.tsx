...
export const InstructorManagement = () => {
  ...
  return (
    <div className='flex flex-col'>
      ...
      <div className='w-max relative rounded-md m-2'>
          ...
          <tbody>
            {instructors.map((instructor: any, index: number) => (
             ...
             even:dark:bg-gray-800 border-b dark:border-gray-700'
              >
                <td scope="row" className=" px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{`${instructor?.instructorId.firstName} ${instructor?.instructorId.lastName}`}</td>
                <td className="px-6 py-4">{instructor?.instructorId.email}</td>
                <td className="px-6 py-4">{new Date(instructor?.instructorId.createdAt).toDateString()}</td>
